...
void dc_rebuild(Codec* dc)
{
    ...
    for (i = 1; i < dc->dict_used; ++i) {
        uint32_t h =
            fnv1a(dc->dict[i]->fmt, strlen(dc->dict[i]->fmt)) % dc->rdict_size;
        while (dc->rdict[h] > 0) {
            ++h;
            if (h == dc->rdict_size)
                h = 0;
...
static inline int
parse_fmt(const char* src, int len, char* fmt, int* flen, int32_t* args)
{
    int         m = 0;  // narg
    bool        hex[20];
    char        num[20][10];
...
static inline int
parse_fmt_new(const char* src, int len, char* fmt, int* flen, uint64_t* args)
{
    int         m = 0;  // narg
    bool        hex[20];
    char        num[20][10];
...
static inline int dc_encode_key_with_fmt(int      idx,
                                         char*    buf,
                                         int      buf_size,
                                         int32_t* args,
                                         int      narg)
...
static inline int dc_encode_key_with_fmt_new(int       idx,
                                             char*     buf,
                                             int       buf_size,
                                             uint64_t* args,
                                             int       narg)
...
static inline int dc_decode_key_with_fmt(Codec*      dc,
                                         char*       buf,
                                         int         buf_size,
                                         const char* src,
                                         int         len)
{
    ...
    int rlen = 0;
    switch (f->nargs) {
        case 1: rlen = safe_snprintf(buf, buf_size, f->fmt, args[0]); break;
        case 2:
            rlen = safe_snprintf(buf, buf_size, f->fmt, args[0], args[1]);
            break;
        case 3:
            rlen =
                safe_snprintf(buf, buf_size, f->fmt, args[0], args[1], args[2]);
            break;
        default:;
    }
    return rlen;
...
static inline int dc_decode_key_with_fmt_new(Codec*      dc,
                                             char*       buf,
                                             int         buf_size,
                                             const char* src,
                                             int         len)
...
int dc_encode(Codec* dc, char* buf, int buf_size, const char* src, int len)
{
    ...
    if (dc && len > 6 && len < 100 && src[0] > 0 && narg > 0 &&
        narg <= MAX_FMT_ARGS) {
        Fmt**    dict = dc->dict;
        ...
        if (rh == 0) {
            if ((unsigned int)(dc->dict_used) < dc->dict_size) {
                dict[dc->dict_used] =
                    (Fmt*)safe_malloc(sizeof(Fmt) + flen - 7 + 1);
