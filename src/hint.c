...
void collect_items(Item* it, void* param)
{
    ...
    r->hash       = it->hash;
    // TODO: check ir
    safe_memcpy(r->key, p->size - p->curr - sizeof(HintRecord) + NAME_IN_RECORD,
                it->key, r->ksize + 1);
...
int count_deleted_record(HTree*      tree,
                         int         bucket,
                         const char* path,
                         int*        total,
                         bool        skipped)
{
    *total         = 0;
    ...
    while (p < end) {
        ...
        if (it == NULL || it->pos != ((r->pos << 8) | (unsigned int)bucket) ||
            (it->ver <= 0 && !skipped)) {
            deleted++;
        }
        if (it)
