...
static void parallelize(HStore* store, BC_FUNC func)
{
    ...
    pthread_t* thread_ids =
        (pthread_t*)safe_malloc(sizeof(pthread_t) * store->scan_threads);
    struct scan_args* args = (struct scan_args*)safe_malloc(
        sizeof(struct scan_args) * store->scan_threads);
    for (i = 0; i < store->scan_threads; i++) {
        args[i].store = store;
        args[i].index = i;
        args[i].func  = func;
        if ((ret = pthread_create(thread_ids + i, &attr, scan_thread,
                                  args + i)) != 0) {
...
HStore* hs_open(char* path, int height, time_t before, int scan_threads)
{
    ...
    HStore* store =
        (HStore*)safe_malloc(sizeof(HStore) + sizeof(Bitcask*) * count);
    if (!store)
        return NULL;
    ...
    for (i = 0; i < count; i++) {
        for (j = 0; j < npath; j++) {
            path = paths[j];
            switch (height) {
                case 0: safe_snprintf(buf[j], MAX_PATH_LEN, "%s", path); break;
                case 1:
                    safe_snprintf(buf[j], MAX_PATH_LEN, "%s/%x", path, i);
                    break;
                case 2:
                    safe_snprintf(buf[j], MAX_PATH_LEN, "%s/%x/%x", path,
                                  i >> 4, i & 0xf);
                    break;
                case 3:
                    safe_snprintf(buf[j], MAX_PATH_LEN, "%s/%x/%x/%x", path,
                                  i >> 8, (i >> 4) & 0xf, i & 0xf);
                    break;
            }
        }
...
char* hs_get(HStore* store, char* key, unsigned int* vlen, uint32_t* flag)
{
    ...
    if (info) {
        ...
        if (info == 2)
            *vlen = (size_t)safe_snprintf(res, META_BUF_SIZE,
                                          "%d %u %u %u %u %u %u", r->version,
                                          hash, r->flag, r->vsz, r->tstamp,
                                          ret_pos & 0xff, ret_pos & 0xffffff00);
        else
            *vlen = (size_t)safe_snprintf(res, META_BUF_SIZE, "%d %u %u %u %u",
                                          r->version, hash, r->flag, r->vsz,
                                          r->tstamp);
...
bool hs_append(HStore* store, char* key, char* value, unsigned int vlen)
{
    ...
    suc =
        hs_set(store, key, body, rlen + vlen, flag, 0);  // TODO: use timestamp
...
void* do_optimize(void* arg)
{
    ...
    time_t  st    = time(NULL);
    log_notice("start to optimize from 0x%x to 0x%x, limit %d", store->op_start,
               store->op_end - 1, store->op_limit);
    store->op_laststat = 0;
    for (; store->op_start < store->op_end && store->op_laststat == 0;
         ++(store->op_start)) {
        store->op_laststat =
    ...
    log_notice("optimization %s in %lld seconds",
               store->op_laststat >= 0 ? "completed" : "failed",
