...
static const long long g_index[]   = {
    0, 1, 17, 273, 4369, 69905, 1118481, 17895697, 286331153, 4581298449L};
...
// forward dec
static void
            add_item(HTree* tree, Node* node, Item* it, uint32_t keyhash, bool enlarge);
...
static inline bool
check_version(Item* oldit, Item* newit, HTree* tree, uint32_t keyhash)
{
    if (abs(newit->ver) >= abs(oldit->ver))
        ...
        log_warn("BUG: bad version, oldv=%d, newv=%d, key=%s, keyhash = 0x%x, "
                 "oldpos = %u",
...
static Item* create_item(HTree*      tree,
                         const char* key,
                         int         len,
                         uint32_t    pos,
                         uint16_t    hash,
                         int32_t     ver)
{
    ...
    it->hash = hash;
    it->ksz =
        dc_encode(tree->dc, it->key,
                  TREE_BUF_SIZE - (sizeof(Item) - ITEM_PADDING), key, len);
    return it;
}
...
static void enlarge_pool(HTree* tree)
{
    ...
    log_notice("enlarge pool %d -> %d, new_height = %d", old_size, new_size,
               tree->height + 1);
...
add_item(HTree* tree, Node* node, Item* it, uint32_t keyhash, bool enlarge)
{
    ...
    for (data = data0; data != NULL; data = data->next) {
        ...
        for (i = 0; i < data->count;
             ++i, p = (Item*)((char*)p + ITEM_LENGTH(p))) {
            if (it->ksz == p->ksz && memcmp(it->key, p->key, it->ksz) == 0) {
    ...
    Data* last  = NULL;
    Data* llast = NULL;
    for (data  = data0; data != NULL && data->size < data->used + it_len;
         llast = last, last = data, data = data->next)
        ;
    ...
    if (node->count > SPLIT_LIMIT) {
        if (node->depth == tree->height - 1) {
            if (enlarge && (tree->height + tree->depth < MAX_DEPTH) &&
                (node->count > SPLIT_LIMIT * 4)) {
    ...
    for (data = data0; data != NULL; last = data, data = data->next) {
        ...
        for (i = 0; i < data->count; ++i, p = (Item*)((char*)p + p_len)) {
            ...
            if (it->ksz == p->ksz && memcmp(it->key, p->key, it->ksz) == 0) {
                ...
                if (data->count > 0) {
                    memmove(p, (char*)p + p_len,
                            data->size - ((char*)p - (char*)data) - p_len);
                }
                else if (data != data0 &&
                         data->next != NULL)  // neither first nor last
                {
                    last->next = data->next;
                    free(data);
                }
...
static void merge_node(HTree* tree, Node* node)
{
    ...
    for (i = 0; i < BUCKET_SIZE; ++i) {
        ...
        for (data = data0; data != NULL; data = data->next) {
            Item* it = data->head;
            for (j = 0; j < data->count;
                 ++j, it = (Item*)((char*)it + ITEM_LENGTH(it))) {
                if (it->ver > 0) {
...
static uint16_t
get_node_hash(HTree* tree, Node* node, const char* dir, unsigned int* count)
{
    if (node->is_node && strlen(dir) > 0) {
        char i = hex2int(dir[0]);
        if (i >= 0) {
            return get_node_hash(tree, get_child(tree, node, i), dir + 1,
                                 count);
        }
        else {
            if (count)
...
static char*
list_dir(HTree* tree, Node* node, const char* dir, const char* prefix)
{
    int dlen = strlen(dir);
    ...
    if (node->is_node) {
        ...
        Node* child = get_child(tree, node, 0);
        if (node->count > 100000 ||
            (prefix == NULL && node->count > SPLIT_LIMIT * 4)) {
            for (i = 0; i < BUCKET_SIZE; i++) {
                Node* t = child + i;
        ...
        for (data = data0; data != NULL; data = data->next) {
            Item* it = data->head;
            for (i = 0; i < data->count;
                 i++, it = (Item*)((char*)it + ITEM_LENGTH(it))) {
                if (dlen > 0) {
                    safe_snprintf(pbuf, 20, "%08x", key_hash(tree, it));
                    if (memcmp(pbuf + tree->depth + node->depth, dir, dlen) !=
                        0) {
                        continue;
                    }
                }
...
static void
visit_node(HTree* tree, Node* node, fun_visitor visitor, void* param)
{
    int i;
    if (node->is_node) {
        Node* child = get_child(tree, node, 0);
        ...
        for (data = data0; data != NULL; data = data->next) {
            ...
            for (i = 0; i < data->count;
                ...
                decode_len =
                    dc_decode(tree->dc, it->key, buf_size, p->key, p->ksz);
...
HTree* ht_open(int depth, int pos, const char* path)
{
    ...
    off_t fsize = 0;
    if (fread(&fsize, sizeof(fsize), 1, f) != 1 || fseeko(f, 0, 2) != 0 ||
        ftello(f) != fsize) {
    ...
    for (i = 0; i < pool_size; i++) {
        ...
        if (size > 0) {
            ...
            if (fread(DATA_file_start(data), size, 1, f) != 1) {
                log_error("load data: size %d fail", size);
                goto FAIL;
            }
            if (data->used != size) {
    ...
    // load Codec
    if (fread(&size, sizeof(int), 1, f) != 1 || size < 0 || size > (10 << 20)) {
...
static int ht_save2(HTree* tree, FILE* f)
{
    ...
    for (i = 0; i < pool_size; i++) {
        ...
        if (data0) {
            ...
            if (fwrite(&(new_data.used), sizeof(int), 1, f) != 1 ||
                fwrite(DATA_file_start(&new_data),
                       DATA_HEAD_SIZE - sizeof(Data*), 1, f) != 1) {
                log_error("write data head failed");
                return -1;
            }
            ...
            for (data = data0; data != NULL; data = data->next) {
                if (data->count == 0)
                    continue;
                if (fwrite(((char*)data) + DATA_HEAD_SIZE,
                           data->used - DATA_HEAD_SIZE, 1, f) != 1) {
...
int ht_save(HTree* tree, const char* path)
{
    ...
    if (ret == 0) {
        ...
        float save_secs = (save_end.tv_sec - save_start.tv_sec) +
                          (save_end.tv_usec - save_start.tv_usec) / 1e6;
        log_notice("save HTree to %s, size = %" PRIu64 ", in %f secs", path,
                   file_size, save_secs);
        mgr_rename(tmp, path);
    }
    else
...
bool check_bucket(HTree* tree, const char* key, int len)
{
    ...
    if (tree->depth > 0 &&
        h >> ((8 - tree->depth) * 4) != (unsigned int)(tree->pos)) {
        log_error("key %s (#%x) should not in this tree (%d:%0x)", key,
                  h >> ((8 - tree->depth) * 4), tree->depth, tree->pos);
        return false;
    }
...
Item* ht_get_withbuf(HTree*      tree,
                     const char* key,
                     int         len,
                     char*       buf,
                     bool        lock)
    ...
    if (NULL != item) {
        ...
        if (tree->updating_bucket == bucket) {
            ...
            if (tree->updating_bucket == bucket) {
                ...
                *is_tmp = 1;
                item    = ht_get_withbuf(tree->updating_tree, key, strlen(key),
                                      buf, false);
            }
            else {
