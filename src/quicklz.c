...
// lar@quicklz.com
//
// QuickLZ can be used for free under the GPL-1, -2 or -3 license (where
// anything released into public must be open source) or under a commercial
// license if such has been acquired (see http://www.quicklz.com/order.html).
// The commercial license does not cover derived or ported versions created by
...
#if QLZ_VERSION_MAJOR != 1 || QLZ_VERSION_MINOR != 4 ||                        \
    QLZ_VERSION_REVISION != 1
...
{
    // Caution if modifying memcpy_up! Overlap of dst and src must be special
    // handled.
#ifndef X86X64
...
static size_t qlz_compress_core(const unsigned char* source,
                                unsigned char*       destination,
                                size_t               size,
                                qlz_hash_compress    hashtable[QLZ_HASH_VALUES],
...
#    ifdef X86X64
            if ((cached & 0xffffff) == 0 && hash_counter[hash] != 0 &&
                (src - o > MINOFFSET ||
                 (src == o + 1 && lits >= 3 && src > source + 3 &&
                  *src == *(src - 3) && *src == *(src - 2) &&
                  *src == *(src - 1) && *src == *(src + 1) &&
                  *src == *(src + 2))))
#    else
            if (cached == 0 && hash_counter[hash] != 0 &&
                (src - o > MINOFFSET ||
                 (src == o + 1 && lits >= 3 && src > source + 3 &&
                  *src == *(src - 3) && *src == *(src - 2) &&
                  *src == *(src - 1) && *src == *(src + 1) &&
...
#    if QLZ_COMPRESSION_LEVEL == 3
                if (((fast_read(o, 3) ^ fetch) & 0xffffff) == 0 &&
                    o < src - MINOFFSET)
#    elif QLZ_COMPRESSION_LEVEL == 2
                if (*(src + matchlen) == *(o + matchlen) &&
                    ((fast_read(o, 3) ^ fetch) & 0xffffff) == 0 &&
                    o < src - MINOFFSET)
#    endif
                {
                    m = 3;
...
static size_t
qlz_decompress_core(const unsigned char* source,
                    unsigned char*       destination,
                    size_t               size,
                    qlz_hash_decompress  hashtable[QLZ_HASH_VALUES],
                    unsigned char        hash_counter[QLZ_HASH_VALUES],
                    ...
                    const char*          source_2)
{
    ...
    for (;;) {
...

        {
            ...
            if (matchlen >
                (ui32)(last_destination_byte - dst - UNCOMPRESSED_END + 1))
                return 0;
#endif
...
size_t qlz_compress(const void* source,
                    char*       destination,
                    size_t      size,
                    char*       scratch_compress)
{
    ...
    qlz_hash_compress* hashtable =
        (qlz_hash_compress*)(scratch_aligned + QLZ_BUFFER_COUNTER);
...
#endif
    {
        ...
        r = base + qlz_compress_core((const unsigned char*)source,
                                     (unsigned char*)destination + base, size,
                                     hashtable, hash_counter);
#if QLZ_STREAMING_BUFFER > 0
        reset_state(hash_counter);
#endif
        if (r == base) {
    ...
    *destination |= (1 << 6);
    *destination |= ((QLZ_STREAMING_BUFFER == 0 ?
                          0 :
...
size_t
qlz_decompress(const char* source, void* destination, char* scratch_compress)
{
    unsigned char* scratch_aligned =
        (unsigned char*)scratch_compress + QLZ_ALIGNMENT_PADD -
        (((size_t)scratch_compress) % QLZ_ALIGNMENT_PADD);
...
#elif QLZ_COMPRESSION_LEVEL <= 2
    qlz_hash_decompress* hashtable =
        (qlz_hash_decompress*)(scratch_aligned + QLZ_BUFFER_COUNTER);
    unsigned char* hash_counter = (unsigned char*)hashtable +
                                  sizeof(qlz_hash_decompress[QLZ_HASH_VALUES]);
#    if QLZ_STREAMING_BUFFER > 0
    unsigned char* streambuffer = hash_counter + QLZ_HASH_VALUES;
#    endif
#endif
...
#endif
    {
        if ((*source & 1) == 1) {
            ...
            dsiz = qlz_decompress_core((const unsigned char*)source + headerlen,
                                       (unsigned char*)destination, dsiz,
                                       hashtable, hash_counter,
        ...
        *buffersize = 0;
        ...
        if ((*source & 1) == 1) {
            dsiz =
                qlz_decompress_core((const unsigned char*)source + headerlen,
                                    streambuffer + *buffersize, dsiz, hashtable,
                                    hash_counter, streambuffer, source);
        }
        else {
