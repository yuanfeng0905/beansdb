...
void compress_record(DataRecord* r)
{
    ...
    if (n > PADDING &&
        (r->flag & (COMPRESS_FLAG | CLIENT_COMPRESS_FLAG)) == 0) {
...
DataRecord* decompress_record(DataRecord* r)
{
    if (r->flag & COMPRESS_FLAG) {
        ...
        if (csize != r->vsz) {
            log_error("broken compressed data: %d != %d, flag=%x", csize,
                      r->vsz, r->flag);
            goto DECOMP_END;
        }
...
DataRecord* decode_record(char*       buf,
                          uint32_t    size,
                          bool        decomp,
                          const char* path,
                          uint32_t    pos,
                          ...
                          int*        fail_reason)
{
    ...
    if (bad_kv_size(ksz, vsz)) {
        if (do_logging)
            log_error("invalid ksz=%u, vsz=%u, %s @%u, key = (%s)", ksz, vsz,
                      path, pos, key);
        if (fail_reason)
    ...
    if (size < need) {
        if (do_logging)
            log_error("not enough data in buffer %d < %d, %s @%u,  key = (%s) ",
                      size, need, path, pos, key);
        if (fail_reason)
            *fail_reason = BAD_REC_END;
        return NULL;
    }
    uint32_t crc = crc32(0, (unsigned char*)buf + sizeof(uint32_t),
                         need - sizeof(uint32_t));
    if (r->crc != crc) {
        if (do_logging)
            log_error("CHECKSUM %u != %u, %s @%u, get (%s) got (%s)", crc,
                      r->crc, path, pos, key, r->key);
        if (fail_reason)
    ...
    memcpy(&r2->crc, &r->crc,
           sizeof(DataRecord) - sizeof(char*) + ksz);  // safe
    r2->key[ksz]   = 0;                                // c str
    r2->free_value = false;
...
static inline DataRecord* scan_record(char*       begin,
                                      char*       end,
                                      char**      curr,
                                      const char* path,
                                      int*        num_broken_total,
                                      HTree*      tree,
                                      int         bucket)
{
    ...
    while (*curr < end) {
        ...
        DataRecord* r = decode_record(p, end - p, false, path, p - begin,
                                      "nokey", do_logging, &bad_reason);
        if (r != NULL) {
            if (num_broken_curr > 0) {
                log_error("END_BROKEN in %s after %d PADDING, total %d", path,
                          num_broken_curr, *num_broken_total);
                num_broken_curr = 0;
            }
            return r;
        }
            ...
            if (num_broken_curr == 0) {
                ...
                uint32_t ksz = ro->ksz;
                if (ksz > 0 && ksz <= MAX_KEY_LEN &&
                    sizeof(DataRecord) - sizeof(char*) + ksz < end - p) {
                    ...
                    if (it && (it->pos & 0xffffff00) == (p - begin) &&
                        ...
                        key[ksz] = 0;
                        log_error("REMOVE_BROKEN key %s in %s at %ld", key,
                                  path, p - begin);
                ...
                if (bad_reason == BAD_REC_CRC) {
                    ...
                    p += jump;
                    DataRecord* rn =
                        decode_record(p, end - p, false, path, p - begin,
                                      "nokey", true, NULL);
                    if (rn != NULL) {
                        *curr = p;
                        jump /= PADDING;
                        ...
                        log_error(
                            "JUMP_BROKEN in %s, jump %d PADDING, total %d",
            ...
            if (num_broken_curr > MAX_VALUE_LEN / PADDING)  // 100M
            {
                ...
                log_error("GIVEUP_BROKEN in %s after %d PADDING, total %d",
    ...
    if (*curr >= end && num_broken_curr > 0) {
        log_error("FILE_END_BROKEN in %s after %d PADDING, total %d", path,
                  num_broken_curr, *num_broken_total);
    }
    return NULL;
}
...
DataRecord* read_record(FILE* f, bool decomp, const char* path, const char* key)
{
    ...
    if (fread(&r->crc, 1, PADDING, f) != PADDING) {
        log_error("read file fail, %s @%lld, key = (%s)", path,
                  (long long int)ftello(f), key);
        goto READ_END;
    }
    ...
    if (vsz < read_size) {
        ...
        int ret  = 0;
        if (need > 0 &&
            need != (ret = fread(r->value + read_size, 1, need, f))) {
            r->key[ksz] = 0;  // c str
            log_error("PREAD %d < %d, %s @%lld, key = (%s)", ret, need, path,
                      (long long int)ftello(f), key);
            goto READ_END;
        }
    }
    ...
    uint32_t crc =
        crc32(0, (unsigned char*)(&r->tstamp),
              sizeof(DataRecord) - sizeof(char*) - sizeof(uint32_t) + ksz);
    crc = crc32(crc, (unsigned char*)r->value, vsz);
    if (crc != crc_old) {
        log_error("CHECKSUM %u != %u, %s @%lld, get key (%s) got(%s)", crc,
                  r->crc, path, (long long int)ftello(f), key, r->key);
        goto READ_END;
    }
...
DataRecord* fast_read_record(int         fd,
                             off_t       offset,
                             bool        decomp,
                             const char* path,
                             const char* key)
{
    DataRecord* r = (DataRecord*)safe_malloc(
        max(sizeof(DataRecord) + MAX_KEY_LEN, PADDING + sizeof(char*)) + 1);
    r->value = NULL;

    if (pread(fd, &r->crc, PADDING, offset) != PADDING) {
        log_error("read file fail, %s @%lld, file size = %lld, key = %s", path,
                  (long long)offset, (long long)lseek(fd, 0L, SEEK_END), key);
        goto READ_END;
    }

    if (bad_kv_size(r->ksz, r->vsz)) {
        log_error("invalid ksz=%u, vsz=%u, %s @%lld, key = (%s)", r->ksz,
                  r->vsz, path, (long long)offset, key);
        goto READ_END;
    }
    int      ksz = r->ksz, vsz = r->vsz;
    uint32_t crc_old = r->crc;
    ...
    if (read_more <= 0) {
        ...
        r->free_value = true;
        int ret       = 0;
        log_warn("long key ksz %d key_more, vsz %d, read_more %d", ksz,
                 key_more, vsz, read_more);
        if (read_more !=
            (ret = pread(fd, r->value, read_more, offset + PADDING))) {
            r->key[ksz] = 0;  // c str
            log_error("PREAD %d < %d, %s @%lld, get key (%s) got(%s)", ret,
                      read_more, path, (long long int)offset, key, r->key);
            goto READ_END;
        }
        ...
        int ret = 0;
        if (read_more !=
            (ret = pread(fd, r->value + vreadn, read_more, offset + PADDING))) {
            r->key[ksz] = 0;  // c str
            log_error("PREAD %d < %d, %s @%lld, get key (%s) got(%s)", ret,
                      read_more, path, (long long int)offset, key, r->key);
            goto READ_END;
    ...
    if (crc != crc_old) {
        log_error("CHECKSUM %u != %u, %s @%lld, get key (%s) got(%s)", crc,
                  r->crc, path, (long long int)offset, key, r->key);
        goto READ_END;
    }
                  ...
                  const char* hintpath)
{
    ...
    while (p < end) {
        ...
        r = decompress_record(r);
        if (r == NULL) {
            log_error("decompress_record fail, %s @%u size = %ld", path, pos,
                      p - (pos + f->addr));
            continue;
        }
...
void scanDataFileBefore(HTree*      tree,
                        int         bucket,
                        const char* path,
                        time_t      before)
{
...
int optimizeDataFile(HTree*      tree,
                     Mgr*        mgr,
                     int         bucket,
                     const char* path,
                     const char* hintpath,
                     ...
                     uint32_t*   deleted_bytes)
{
    ...
    int err = -1;
    log_notice("begin optimize %s -> %s, use_tmp = %s", path, lastdata,
               use_tmp ? "true" : "false");
    ...
    if (!use_tmp) {
        ...
        if (end != 0) {
            char bytes[256];
            int  size = 256 - end;
            log_warn("size of %s is 0x%llx, add padding", lastdata,
                     (long long)new_df_orig_size);
    ...
    while (p < end) {
        DataRecord* r =
            scan_record(f->addr, end, &p, path, &broken, tree, bucket);
        if (r == NULL) {
            if (p < end)
                goto OPT_FAIL;
            break;
        ...
        if (it && it->pos == (pos | bucket) && (it->ver > 0 || skipped)) {
            ...
            if (new_pos + record_length(r) > max_data_size) {
                if (use_tmp) {
                    log_warn("Bug: optimize %s into tmp %s overflow", path,
                             tmp);
                }
                else {
                    log_warn("optimize %s into %s overflow, ftruncate to %u",
                             path, lastdata, new_df_orig_size);
                    fflush(new_df);
                    if (0 != ftruncate(fileno(new_df), new_df_orig_size)) {
                        log_error("ftruncate failed for  %s old size = %u",
            ...
            ht_add2(cur_tree, r->key, r->ksz, new_pos | last_bucket, hash,
                    it->ver);
            ...
            hr->hash       = hash;
            safe_memcpy(hr->key,
                        hint_size - sizeof(uint32_t) - sizeof(int32_t) -
                            sizeof(uint16_t),
                        r->key, r->ksz + 1);
            hint_used += hsize;
    ...
    *deleted_bytes = f->size - (ftello(new_df) - new_df_orig_size);
    ...
    float update_secs = (update_end.tv_sec - update_start.tv_sec) +
                        (update_end.tv_usec - update_start.tv_usec) / 1e6;
    float opt_secs = (opt_end.tv_sec - opt_start.tv_sec) +
                     (opt_end.tv_usec - opt_start.tv_usec) / 1e6;
    log_notice("optimize %s -> %d (%u B) complete, %d/%d records released, %d "
               "deleted, %u/%u bytes released, %d bytes broken, use %fs/%fs",
               path, last_bucket,
               (last_bucket == bucket) ? old_srcdata_size : new_df_orig_size,
               released, nrecord, deleted, *deleted_bytes, old_srcdata_size,
               broken, update_secs, opt_secs);
    return 0;

OPT_FAIL:
    log_notice(
        "optimize %s -> %d (%u B) failed,   %d/%d records released, %d "
        "deleted, %u/%u bytes released, %d bytes broken, use %fs/%fs, err = %d",
        path, last_bucket,
        (last_bucket == bucket) ? old_srcdata_size : new_df_orig_size, released,
        nrecord, deleted, *deleted_bytes, old_srcdata_size, broken, update_secs,
        opt_secs, err);
    if (hintdata)
        free(hintdata);
    if (cur_tree)
